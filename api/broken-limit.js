...
async function push2exBrokenLimit(date) {
  ...
  const data = await fetchJson(
    `https://push2ex.eastmoney.com/getTopicZBPool?ut=${UT}&dpt=${DPT}&Pageindex=0&pagesize=200&sort=fbt:asc&date=${targetDate}&_=${Date.now()}`,
    H
  );
...
module.exports = async (req, res) => {
  ...
  try {
    const [xgb, push] = await Promise.all([
      xgbBrokenLimit().catch(() => null),
      push2exBrokenLimit(date).catch(() => null),
    ]);
