...
const tools = [
  tool('get_daily_review_bundle', '获取一键短线复盘聚合包', '一键聚合今日短线复盘所需数据。当前默认返回增强包：基础盘口数据 + 核心票概念、资金流、新闻公告、人气榜。适用于“复盘今天”“开搞”“今天盘面怎么变化”。', schema({ date: { type: 'string', description: '日期 YYYYMMDD。' }, group: { type: 'string', description: '观察池组：default、semiconductor、robot、ai_compute、innovation_drug、fluorochemical、paper、market_core。' }, symbols: { type: 'string', description: '自定义核心观察票，多个逗号分隔。' }, raw: { type: 'boolean', description: '是否返回原始完整数据。' }, extra: { type: 'boolean', description: '是否返回增强信号；默认 true。' } })),
